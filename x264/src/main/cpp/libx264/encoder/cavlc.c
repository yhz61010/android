...
static ALWAYS_INLINE void cavlc_macroblock_luma_residual(x264_t *h, int plane_count) {
    ...
    for (int p = 0; p < plane_count; p++)
        FOREACH_BIT(i8, 0, h->mb.i_cbp_luma)for (int i4 = 0; i4 < 4; i4++) x264_cavlc_block_residual(h, DCT_LUMA_4x4, i4 + i8 * 4 + p * 16, h->dct.luma4x4[i4 + i8 * 4 + p * 16]);
}
...
void x264_macroblock_write_cavlc(x264_t *h) {
    ...
    if (i_mb_type == I_16x16) {
        ...
        for (int p = 0; p < plane_count; p++) {
            ...
            /* AC Luma */
            if (h->mb.i_cbp_luma)
                for (int i = p * 16; i < p * 16 + 16; i++) x264_cavlc_block_residual(h, DCT_LUMA_AC, i, h->dct.luma4x4[i] + 1);
        }
    ...
    if (h->mb.i_cbp_chroma) {
        ...
        if (h->mb.i_cbp_chroma == 2) /* Chroma AC residual present */
        {
            ...
            for (int i = 16; i < 3 * 16; i += step)
                for (int j = i; j < i + 4; j++) x264_cavlc_block_residual(h, DCT_CHROMA_AC, j, h->dct.luma4x4[j] + 1);
        }
    }
