...
fun printTree(root: TreeElement, indent: String, isLast: Boolean, printCallback: (String) -> Unit) {
    printCallback(
        indent + (if (isLast) BOX_DRAWING_CHAR_LAST else BOX_DRAWING_CHAR_MIDDLE) + root.name
    )
    ...
    if (fixedChildren?.isNotEmpty() == true) {
        fixedChildren.forEachIndexed { index, child ->
            ...
            val childIndent =
                indent + (if (isLast) BOX_DRAWING_CHAR_SPACE else BOX_DRAWING_CHAR_LINE)
...
fun printFileTree(file: File, indent: String, isLast: Boolean, printCallback: (String) -> Unit) {
    printCallback(
        indent + (if (isLast) BOX_DRAWING_CHAR_LAST else BOX_DRAWING_CHAR_MIDDLE) + file.name
    )
