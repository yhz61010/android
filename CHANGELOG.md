...
- **`MicRecorder` 检查音效启用结果，并在构造失败时回滚**：`initAdvancedFeatures()` 原先用
  `enabled = true` 开启三个音效，这会丢弃 `AudioEffect.setEnabled()` 返回的状态码。被平台拒绝开启的
  音效会照样留在字段里、被当作已生效，双向语音于是在没有回声消除的情况下继续运行，日志里也没有
  任何线索。现改为显式判断 `setEnabled(true) == AudioEffect.SUCCESS`，未能开启的音效立即
  `release()`、字段保持 `null`，使 `null` 仍然只表示「该音效未生效」。
  另外构造函数此前没有回滚：`AudioRecord` 构造或音效挂载抛异常时，已创建的编码器、`AudioRecord`
  和音效会随构造失败一起滞留——调用方拿不到对象引用，也就无从调用 `stopRecordAndJoin()`，只能等
  GC 终结器回收。现新增回滚，先释放已建成的部分，再把原异常抛出。
...
- **screencapture 初始化中断与输出失败收尾**：等待 `onInit()` 的线程被中断时，恢复其
  ...
  而未创建录制协程的路径，并在清理后通知错误。新增三个回归用例。
  `onInit()` 增加 `@Throws(InterruptedException::class)`：Kotlin 侧签名与行为不变，但 JVM 签名
  现在如实声明该受检异常，此前 Java 调用方无法为它编译出 `catch`，却可能在运行时收到它。
...
- **Screenshot H.26x 初始化失败不再崩溃**：EGL config/context/window surface、input Surface 或编码器
  ...
  **修复**：`Record Single App Screen` 停止录制后开关按钮永久禁用。该策略是一次性的，Demo 却
  ...
  路径恢复可用状态。现在清理完成后会重建录制器并重新启用按钮，可反复录制；退出页面时不再重建。
  调试输出流在每次开始录制时才打开（打开即截断文件），停止后不再重开，避免刚录完的文件被清零。
...
- **camera2live 横屏照片方向与镜像**:拍照请求使用物理方向监听结果设置 `JPEG_ORIENTATION`,不再只依赖可能
  未随持机方向变化的 `Display.rotation`;普通 JPEG 保存时按 CameraX 相同语义执行像素级旋转/水平镜像后重写,
  镜头切换后同步刷新方向源,并修正未知方向与 316°–359° 的监听边界;前后置真机回归已通过。
- **camera2live 裸 H.264 横屏方向与前置颜色**:录像开始时锁定设备物理相对方向,前后置 YUV 按
  0°/90°/180°/270°旋转并以旋转后的宽高初始化 AVC 编码器;前置只使用合法 I420 旋转/镜像并显式输出目标
  I420/NV12 格式,不再用 NV21 函数处理 I420 导致偏色。裸码流不依赖容器旋转元数据或黑边填充;前后置真机
  回归已通过。
