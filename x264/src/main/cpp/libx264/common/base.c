...
#define HUGE_PAGE_THRESHOLD HUGE_PAGE_SIZE*7/8 /* FIXME: Is this optimal? */
    if (i_size < 0 || (uint64_t) i_size > (SIZE_MAX - HUGE_PAGE_SIZE) /*|| (uint64_t)i_size > (SIZE_MAX - NATIVE_ALIGN - sizeof(void **))*/ ) {
        x264_log_internal(X264_LOG_ERROR, "invalid size of malloc: %"
        PRId64
        "\n", i_size );
        return NULL;
    }
...
#endif
    if (!align_buf)
        x264_log_internal(X264_LOG_ERROR, "malloc of size %"
    PRId64
    " failed\n", i_size );
    return align_buf;
...
REALIGN_STACK int x264_picture_alloc(x264_picture_t *pic, int i_csp, int i_width, int i_height) {
    ...
    for (int i = 0; i < pic->img.i_plane; i++) {
        int stride = (((int64_t)
        i_width * csp_tab[csp].width_fix8[i]) >> 8) *depth_factor;
        int64_t
        plane_size = (((int64_t)
        i_height * csp_tab[csp].height_fix8[i]) >> 8) *stride;
...
#define OPT2(STR0, STR1) else if( !strcmp( name, STR0 ) || !strcmp( name, STR1 ) )
    ...
    OPT("avcintra-flavor")b_error |= parse_enum(value, x264_avcintra_flavor_names, &p->i_avcintra_flavor);
    OPT("sar") {
    ...
    } OPT("alternative-transfer")b_error |= parse_enum(value, x264_transfer_names, &p->i_alternative_transfer);
    OPT("fps") {
    ...
    OPT2("direct", "direct-pred")b_error |= parse_enum(value, x264_direct_pred_names, &p->analyse.i_direct_mv_pred);
    ...
    OPT("crop-rect")b_error |= sscanf(value, "%d,%d,%d,%d", &p->crop_rect.i_left, &p->crop_rect.i_top,
