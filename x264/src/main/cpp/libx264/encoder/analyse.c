...
static void mb_analyse_init(x264_t *h, x264_mb_analysis_t *a, int qp) {
    ...
    uint64_t
    pcm_cost = ((uint64_t)
    X264_PCM_COST * a->i_lambda2 + 128) >> 8;
        ...
        if (a->b_early_terminate && h->mb.i_mb_xy - h->sh.i_first_mb > 4) {
            if (IS_INTRA(h->mb.i_mb_type_left[0]) ||
                ...
                (h->mb.i_mb_xy - h->sh.i_first_mb < 3 * (h->stat.frame.i_mb_count[I_4x4] + h->stat.frame.i_mb_count[I_8x8] + h->stat.frame.i_mb_count[I_16x16] +
                                                         h->stat.frame.i_mb_count[I_PCM]))) { /* intra is likely */ }
            else {
                a->b_fast_intra = 1;
            }
        }
...
{ \
    ...
    if (i_pixel == PIXEL_16x16) COST_BI_CHROMA(a->l0.bi16x16, a->l1.bi16x16, 16, 16)
    else if (i_pixel == PIXEL_16x8) COST_BI_CHROMA(a->l0.me16x8[idx], a->l1.me16x8[idx], 16, 8)
    else if (i_pixel == PIXEL_8x16) COST_BI_CHROMA(a->l0.me8x16[idx], a->l1.me8x16[idx], 8, 16)
    else COST_BI_CHROMA(a->l0.me8x8[idx], a->l1.me8x8[idx], 8, 8)
