 ...
 */
JNIEXPORT jint JNICALL init(JNIEnv *env, jobject obj, jbyteArray vpsByteArray, jbyteArray spsByteArray, jbyteArray ppsByteArray) {
    LOGE("H264 & HEVC decoder init.");

    AVCodecID codecId = AV_CODEC_ID_H264;

    int vpsLen = 0;
    uint8_t *vps_unit8_t_array = NULL;
    if (NULL != vpsByteArray) {
...
//
//    int csdLen = vpsLen + spsLen + ppsLen;
//    auto *csd_array = new uint8_t[csdLen];
//    if (NULL != vps_unit8_t_array) {
...
//
//    if (NULL != vps_unit8_t_array) free(vps_unit8_t_array);
...
//    free(csd_array);
    ...
    LOGE("Video meta=%s width=%d height=%d", buf, ctx->width, ctx->height);
