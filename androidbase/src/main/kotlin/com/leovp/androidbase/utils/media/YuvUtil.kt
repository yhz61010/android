...
object YuvUtil {
    ...
    @Deprecated("Unused in this library and scheduled for removal in a future major version.")
    ...
    fun rotateYUV420Degree90(imageBytes: ByteArray, imageWidth: Int, imageHeight: Int): ByteArray {
        ...
        val yuv =
            ByteArray(
                imageWidth * imageHeight * ImageFormat.getBitsPerPixel(
    ...
    // Works - Convert 420_888 to NV21 directly, so that we can use generated bytes to encode into
    // H264
    @Deprecated("Unused in this library and scheduled for removal in a future major version.")
    ...
    fun i420ToRGBABitmap(i420Data: ByteArray, width: Int, height: Int): Bitmap? {
        ...
        for (i in 0 until height) {
            for (j in 0 until width) {
                ...
                val u =
                    0xff and i420Data[frameSize + (i shr 1) * width + (j and 1.inv()) + 0].toInt()
                val v =
                    0xff and i420Data[frameSize + (i shr 1) * width + (j and 1.inv()) + 1].toInt()
                y = if (y < 16) 16 else y
    ...
    // private static Bitmap YUV_420_888_toRGBIntrinsics(Context context, int width, int height,
    // byte[] yuv) {
    ...
    //    // Very slow
    // //
    // https://blog.csdn.net/tklwj/article/details/87975528?depth_1-utm_source=distribute.pc_relevan
    // t.none-task&utm_source=distribute.pc_relevant.none-task
    ...
    // LogContext.log.d(TAG, "image: " + imgYUV420.getWidth() + "x" + imgYUV420.getHeight() + " " +
    // imgYUV420.getFormat());
    ...
    // imgYUV420.getPlanes()[nPlane].getBuffer().remaining() +
// ", strides: " + imgYUV420.getPlanes()[nPlane].getPixelStride() + " " +
// imgYUV420.getPlanes()[nPlane].getRowStride());
    ...
    //            }
    //        }
    // assert (imgYUV420.getPlanes()[2].getPixelStride() ==
    // imgYUV420.getPlanes()[1].getPixelStride());
