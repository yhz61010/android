 ...
 * (if any).
 * Base64.NO_WRAP -> Encoder flag bit to omit all line terminators (i.e., the output will be on one
 * long line).
 * Base64.URL_SAFE -> Encoder/decoder flag bit to indicate using the "URL and filename safe" variant
 * of Base64
 * (see RFC 3548 section 4) where `-` and `_` are used in place of `+` and '/'.
 */
@JvmOverloads
fun String.toBase64(flag: Int = Base64.DEFAULT): String =
    Base64.encodeToString(this.toByteArray(), flag)
