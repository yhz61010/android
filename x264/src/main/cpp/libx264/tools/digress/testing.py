...
.
errors import
SkippedTestError, DisabledTestError, NoSuchTestError, \
...
:
func.
digress_depends = self.test_names
return
func
...
:
func.
digress_comparer = self._comparer
return
func
...
if depend.
__name__ in
results andresults[depend.__name__]["status"]
!= CASE_PASS:
...
try:
rmtree(os
.path.join(".digress_%s" % self.__class__.__name__, rev))
except Exception, e
:
print "failed: %s" % e
else:
