...
av_unused get_cabac_noinline(CABACContext *c, uint8_t *const state) {
    return get_cabac_inline(c, state);
}

static int av_unused
get_cabac(CABACContext
*c,
uint8_t *const state
){
return
get_cabac_inline(c, state
);
}

#ifndef get_cabac_bypass
static int av_unused
get_cabac_bypass(CABACContext
*c){
int range;
c->low += c->
low;
...
#ifndef get_cabac_terminate
static int av_unused
get_cabac_terminate(CABACContext
*c){
c->range -= 2;
 ...
 */
#ifndef skip_bytes
static av_unused const uint8_t
*
skip_bytes(CABACContext
*c,
int n
) {
...
NULL;
if (
ff_init_cabac_decoder(c, ptr
+ n, c->bytestream_end - ptr - n) < 0)
return
NULL;
