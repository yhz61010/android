...
static int rd_cost_mb(x264_t *h, int i_lambda2) {
    ...
    if (IS_SKIP(h->mb.i_type)) {
        ...
        i_bits = ((uint64_t)
        cabac_tmp.f8_bits_encoded * i_lambda2 + 32768 ) >> 16;
    } else {
        macroblock_size_cavlc(h);
        i_bits = ((uint64_t)
        h->out.bs.i_bits_encoded * i_lambda2 + 128 ) >> 8;
    }
...
static uint64_t rd_cost_subpart(x264_t *h, int i_lambda2, int i4, int i_pixel) {
    ...
    if (CHROMA444) {
        ...
        chromassd = ((uint64_t)
        chromassd * h->mb.i_chroma_lambda2_offset + 128) >> 8;
        i_ssd += chromassd;
    }
    ...
    if (h->param.b_cabac) {
        ...
        i_bits = ((uint64_t)
        cabac_tmp.f8_bits_encoded * i_lambda2 + 128 ) >> 8;
    } else
...
uint64_t x264_rd_cost_part(x264_t *h, int i_lambda2, int i4, int i_pixel) {
    ...
    if (h->param.b_cabac) {
        ...
        i_bits = (uint64_t)
    partition_size_cavlc(h, i8, i_pixel) * i_lambda2;
...
static uint64_t rd_cost_i8x8(x264_t *h, int i_lambda2, int i8, int i_mode, pixel edge[4][32]) {
    ...
    if (h->param.b_cabac) {
        ...
        i_bits = (uint64_t)
    partition_i8x8_size_cavlc(h, i8, i_mode) * i_lambda2;
...
static uint64_t rd_cost_i4x4(x264_t *h, int i_lambda2, int i4, int i_mode) {
    ...
    if (h->param.b_cabac) {
        ...
        i_bits = (uint64_t)
    partition_i4x4_size_cavlc(h, i4, i_mode) * i_lambda2;
