...
JNICALL init(JNIEnv *env, jobject obj, jint sampleRate, jint channels) {
...
JNICALL chunkSize(JNIEnv *env, jobject obj) {
...
JNICALL decode(JNIEnv * env, jobject
obj,
jbyteArray adpcmByteArray
) {
...
env->
GetByteArrayRegion(adpcmByteArray,
0, adpcmLen, reinterpret_cast<jbyte *>(adpcm_unit8_t_array));
...
env->
SetByteArrayRegion(pcm_byte_array,
0, pcmSize, reinterpret_cast
<const jbyte *>(pcm_all_channel_data)
);
return
pcm_byte_array;
} else { // For mono
...
JNIEXPORT jstring
JNICALL getVersion(JNIEnv * env, jobject
thiz) {
