...
This repository is a multi-module Android project built with Gradle Kotlin DSL. Shared build logic lives in [build.gradle.kts](/home/yhz61010/StudioProjects/android/build.gradle.kts), [settings.gradle.kts](/home/yhz61010/StudioProjects/android/settings.gradle.kts), and [gradle/libs.versions.toml](/home/yhz61010/StudioProjects/android/gradle/libs.versions.toml). Feature and library modules sit at the repo root, for example `androidbase`, `audio`, `camerax`, `lib-network`, `lib-image`, and `lib-compose`. The `demo` app is the main integration surface for manual verification. Native code lives under `src/main/cpp`, and Android resources live under `src/main/res`.
...
- `./gradlew test`: run JVM unit tests across modules that define `src/test`.
