...
for (
w2 = 0;
w2<sce->ics.group_len[w]; w2++) {
FFPsyBand *band = &s->psy.ch[s->cur_channel].psy_bands[(w + w2) * 16 + g];
if (start >= cutoff || band->energy <= (band->
threshold *zeroscale
) || band->threshold == 0.0f) {
...
SCALE_ONE_POS
+ 1.75*
log2f(FFMAX(0.00125f, uplims[w * 16 + g])
/ sce->ics.swb_sizes[g])
+ sfoffs,
60, SCALE_MAX_POS);
...
sce->sf_idx[w*16+g] =
av_clip(sce
->sf_idx[w*16+g], minscaler, minscaler + SCALE_MAX_DIFF - 1);

if (!allz)
return;
s->
abs_pow34(s
->scoefs, sce->coeffs, 1024);
ff_quantize_band_cost_cache_init(s);

for (
i = 0;
i < sizeof(minsf) / sizeof(minsf[0]); ++i)
minsf[i] = 0;
for (
w = 0;
w<sce->ics.
...
maxvals[w*16+g] =
find_max_val(sce
->ics.group_len[w], sce->ics.swb_sizes[g], scaled);
if (maxvals[w*16+g] > 0) {
...
for (
i = 0;
i < sizeof(maxsf) / sizeof(maxsf[0]); ++i)
...
num_windows;
w += sce->ics.group_len[w]) {
if (!sce->zeroes[w*16+g] && sce->can_pns[w*16+g] && spread_thr_r[w*16+g] <=
zspread
        &&sce
->sf_idx[w*16+g] >
loopminsf
        &&(dists[w * 16 + g]
>
loopovrfactor *uplims[w * 16 + g]
|| !(
mcb = find_min_book(maxvals[w * 16 + g], sce->sf_idx[w * 16 + g])
)
|| (mcb <= 1 && dists[w*16+g] >
...
if ((!cmb || dists[w*16+g] > uplims[w*16+g]) && sce->sf_idx[w*16+g] >
...
for (
i = 0;
i<edepth && sce->sf_idx[w * 16 + g]> mindeltasf;
++i) {
int cb, bits;
float dist, qenergy;
...
if (!
g &&sce
->ics.num_windows > 1 && dists[w*16+g] >= euplims[w*16+g])
maxsf[w*16+g] =
FFMIN(sce
...
(dists[w*16+g] <
FFMIN(uplmax
*uplims[w*16+g], euplims[w*16+g]))
&& (
fabsf(qenergies[w * 16 + g]
-energies[w*16+g]) < euplims[w*16+g])
))) {
break;
}
}
...
bits;
if (
dist< FFMIN(euplims[w * 16 + g], uplims[w * 16 + g])
) {
sce->sf_idx[w*16+g]++;
dists[w*16+g] =
dist;
...
sce->sf_idx[w*16+g] =
av_clip(sce
->sf_idx[w*16+g], prev - SCALE_MAX_DIFF, prev + SCALE_MAX_DIFF);
sce->band_type[w*16+g] =
find_min_book(maxvals[w * 16 + g], sce
->sf_idx[w*16+g]);
prev = sce->sf_idx[w * 16 + g];
...
if (!
ff_sfdelta_can_remove_band(sce, nextband, prev, w
*16+g)) {
