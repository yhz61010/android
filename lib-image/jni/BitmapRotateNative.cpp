...
void convertIntToArgb(uint32_t
pixel,
ARGB *argb
) {
argb ->
...
if (jniBitmap == NULL || jniBitmap->_storedBitmapPixels == NULL) return;
...
JNIEXPORT jobject
GetBitmapFromStoredBitmapData(JNIEnv
* env,
jobject obj, jobject
handle) {
JniBitmap *jniBitmap = (JniBitmap *) env->GetDirectBufferAddress(handle);
...
uint32_t *newBitmapPixels = (uint32_t * )
bitmapPixels;
...
JNIEXPORT jobject
JNICALL SetBitmapData(JNIEnv * env, jobject
obj,
jobject bitmap
) {
...
JNIEXPORT void JNICALL
ScaleNNBitmap(JNIEnv
* env,
jobject obj,
        jobject
handle,
uint32_t newWidth, uint32_t
newHeight) {
JniBitmap *jniBitmap = (JniBitmap *) env->GetDirectBufferAddress(handle);
if (jniBitmap == NULL || jniBitmap->_storedBitmapPixels == NULL) return;
uint32_t oldWidth = jniBitmap->_bitmapInfo.width;
...
rgbBottomMiddle.
alpha = rgbBottomLeft.alpha * xcRatio2 + rgbBottomRight.alpha * xcratio1;
rgbBottomMiddle.
red = rgbBottomLeft.red * xcRatio2 + rgbBottomRight.red * xcratio1;
rgbBottomMiddle.
green = rgbBottomLeft.green * xcRatio2 + rgbBottomRight.green * xcratio1;
rgbBottomMiddle.
blue = rgbBottomLeft.blue * xcRatio2 + rgbBottomRight.blue * xcratio1;
...
rgbTopMiddle.
alpha = rgbTopLeft.alpha * xcRatio2 + rgbTopRight.alpha * xcratio1;
rgbTopMiddle.
red = rgbTopLeft.red * xcRatio2 + rgbTopRight.red * xcratio1;
rgbTopMiddle.
green = rgbTopLeft.green * xcRatio2 + rgbTopRight.green * xcratio1;
rgbTopMiddle.
blue = rgbTopLeft.blue * xcRatio2 + rgbTopRight.blue * xcratio1;
//rgbBottomLeft=startingImageData[xTopLeft][yTopLeft+1];
convertIntToArgb(previousData[((yTopLeft + 1) * oldWidth) + xTopLeft], &rgbBottomLeft
);
//rgbBottomRight=startingImageData[xTopLeft+1][yTopLeft+1];
...
result.
alpha = rgbTopMiddle.alpha * ycRatio2 + rgbBottomMiddle.alpha * ycRatio1;
result.
blue = rgbTopMiddle.blue * ycRatio2 + rgbBottomMiddle.blue * ycRatio1;
result.
red = rgbTopMiddle.red * ycRatio2 + rgbBottomMiddle.red * ycRatio1;
result.
green = rgbTopMiddle.green * ycRatio2 + rgbBottomMiddle.green * ycRatio1;
newBitmapPixels[(
y *newWidth
) + x] =
...
if (jniBitmap == NULL || jniBitmap->_storedBitmapPixels == NULL) return;
uint32_t *previousData = jniBitmap->_storedBitmapPixels;
int width = jniBitmap->_bitmapInfo.width, middle =
        width / 2, height = jniBitmap->_bitmapInfo.height;
for (
int y = 0;
y<height;
++y) {
...
if (jniBitmap == NULL || jniBitmap->_storedBitmapPixels == NULL) return;
uint32_t *previousData = jniBitmap->_storedBitmapPixels;
int width = jniBitmap->_bitmapInfo.width, height = jniBitmap->_bitmapInfo.height, middle =
        height / 2;
for (
int y = 0;
y<middle;
...
static JNINativeMethod methods[] = {
        ...
        {"getBitmapFromStoredBitmapData", "(Ljava/nio/ByteBuffer;)Landroid/graphics/Bitmap;", (void *) GetBitmapFromStoredBitmapData},
