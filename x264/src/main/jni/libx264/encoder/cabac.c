...
static ALWAYS_INLINE void cabac_mb_header_b(x264_t *h, x264_cabac_t *cb, int i_mb_type, int chroma) {
    ...
    if (i_mb_type == B_8x8) {
        ...
        x264_cabac_encode_decision(cb, 27 + 5 - (bits & 1), (bits >> 1) & 1);
        bits >>= 2;
        if (bits != 1) {
            x264_cabac_encode_decision(cb, 27 + 5, bits & 1);
            bits >>= 1;
            x264_cabac_encode_decision(cb, 27 + 5, bits & 1);
            bits >>= 1;
            x264_cabac_encode_decision(cb, 27 + 5, bits & 1);
...
{\
    ...
    MUNGE( M32( &nnzbak[2][4] ), M32( &h->mb.cache.non_zero_count[x264_scan8[16*2] - 8] ), 0x00000000U )\
                ...
                for (int p = 0; p < 3; p++)
                    FOREACH_BIT(i, 0, h->mb.i_cbp_luma)cabac_block_residual_8x8_cbf(h, cb, ctx_cat_plane[DCT_LUMA_8x8][p], i * 4 + p * 16, h->dct.luma8x8[i + p * 4], b_intra);

                MUNGE_8x8_NNZ(RESTORE)
            } else {
                FOREACH_BIT(i, 0, h->mb.i_cbp_luma)cabac_block_residual_8x8(h, cb, DCT_LUMA_8x8, h->dct.luma8x8[i]);
            }
        } else {
            for (int p = 0; p < plane_count; p++)
                FOREACH_BIT(i8x8, 0, h->mb.i_cbp_luma)for (int i = 0; i < 4; i++)
